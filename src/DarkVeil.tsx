...
/** React Bits 原版 fragment — 换色只靠 uHueShift */
const fragment = `
#ifdef GL_ES
...
/**
 * hueShift=0 → 官网紫（约 290°）。
 * 正方向 → 青绿；负向 → 品红/红/橙。
 * 爱马仕橙实测：裸算约 -95 偏红，再偏 -40 到橙。
 */
const VEIL_BASE_HUE = 290;
const WARM_BIAS = -40;

function parseCssColor(raw: string): [number, number, number] {
  const s = raw.trim();
  if (!s) return [1, 0.416, 0];
  if (s.startsWith("#")) {
    const h = s.slice(1);
    const full =
      h.length === 3
    ...
    if (Number.isNaN(n)) return [1, 0.416, 0];
...
/** 读 CSS --accent → React Bits 的 hueShift（负向对齐暖色） */
export function hueShiftFromAccentVar(): number {
  const raw =
    getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() ||
    "#ff6a00";
  const [r, g, b] = parseCssColor(raw);
  // 基准紫 − 强调色（负向）+ 暖色微调（红 → 橙）
  let shift = VEIL_BASE_HUE - rgbToHueDeg(r, g, b) + WARM_BIAS;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uHueShift: {
          value: hueShift !== undefined ? hueShift : hueShiftFromAccentVar(),
        },
    ...
    const loop = () => {
      ...
      program.uniforms.uHueShift.value =
        p.hueShift !== undefined ? p.hueShift : hueShiftFromAccentVar();
