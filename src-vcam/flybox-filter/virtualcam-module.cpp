...
/* ComPtr from libdshowcapture (via dshow-base.hpp) */
...
static bool RegServers(bool reg)
{
	...
	if (reg) {
		return RegServer(CLSID_OBS_VirtualVideo, L"FLYBOX Camera", file);
	} else {
...
static bool RegFilters(bool reg)
{
	...
	if (reg) {
		...
		hr = fm->RegisterFilter(CLSID_OBS_VirtualVideo, L"FLYBOX Camera", &moniker,
